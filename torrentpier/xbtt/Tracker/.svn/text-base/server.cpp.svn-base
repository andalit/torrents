...
void Cserver::write_db_users()
{
	...
	if (!m_users_updates_buffer.empty())
	{
		...
		try
		{
			m_database.query("insert into " + table_name(table_users) + " (u_down_total, u_up_total, " + column_name(column_users_uid) + ", u_up_release, u_up_bonus, max_up_speed, max_down_speed) values "
				...
				+ "  u_down_yday = u_down_yday + values(u_down_total),"
				+ "  u_up_yday = u_up_yday + values(u_up_total),"
				+ "  u_release_yday = u_release_yday + values(u_up_release),"
				+ "  u_bonus_yday = u_bonus_yday + values(u_up_bonus),"
				+ "  u_up_speed_yday = GREATEST(u_up_speed_yday, values(max_up_speed)),"
				+ "  u_down_speed_yday = GREATEST(u_down_speed_yday, values(max_down_speed))");
		}
		catch (Cdatabase::exception&)
		{
		}
	...
	if (!m_tor_dl_stat_buffer.empty())
	{
		...
		try
		{
			m_database.query("insert into bb_bt_tor_dl_stat (topic_id,user_id,attach_id,t_up_total,t_down_total,t_bonus_total) values"
...
int Cserver::test_sql()
{
	...
	try
	{
		...
		// TorrentPier begin
		m_database.query("select topic_id, peer_hash, user_id, ip, ipv6, port, uploaded, downloaded, complete_percent, seeder, speed_up, speed_down, update_time, ul_gdc, ul_gdc_c, ul_16k_c, ul_eq_dl from " + table_name(table_files_users) + " where 0"); // Note: `port_open` is not used any more
		m_database.query("select " + column_name(column_users_uid) + ", auth_key, "
                  ...
                  + " as u_tl, u_down_total, u_up_total, u_up_bonus, u_down_today, u_up_today, u_bonus_today, max_up_speed, max_down_speed from " + table_name(table_users) + " where 0");
