...
function tracker_exit ()
{
	...
	if (DBG_LOG)
	{
		...
		$num_queries    = !empty($db) ? $db->num_queries : '-';
		$sql_inittime   = !empty($db) ? $db->sql_inittime : '  --  ';
		$sql_timetotal  = !empty($db) ? $db->sql_timetotal : '  --  ';
		$sql_init_perc  = !empty($db) ? round($sql_inittime*100/$gen_time) : ' - ';
		$sql_total_perc = !empty($db) ? round($sql_timetotal*100/$gen_time) : ' - ';
