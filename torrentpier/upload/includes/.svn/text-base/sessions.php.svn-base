...
switch ($bb_cfg['session_cache_type'])
{
	...
	case 'eaccelerator':
		$session_cache = new cache_eaccelerator();
		break;
		
	case 'APC':
		$session_cache = new cache_apc();
		break;
		
	case 'memcached':
		$session_cache = new cache_memcached($bb_cfg['session_cache']['memcached']);
		break;
		
	case 'xcache':
		$session_cache = new cache_xcache();
		break;
...
function cache_set_userdata ($userdata, $force = false)
{
	...
	return $session_cache->set($id, $userdata, $bb_cfg['session_update_intrv']);
}
...
function cache_rm_user_sessions ($user_id)
{
	...
	{
		$session_cache->rm($row['session_id']);
	}
}
