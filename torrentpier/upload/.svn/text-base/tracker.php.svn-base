...
if (!IS_GUEST)
{
	$sql = "SELECT tor_search_set, last_modified FROM ". BT_USER_SETTINGS_TABLE ." WHERE user_id = $user_id LIMIT 1";
...
if (!$set_default)
{
	...
	{
		if ($tmp = substr(trim($_REQUEST[$title_match_key]), 0, $title_match_max_len))
		{
			...
			if (SPHINX_ENABLE)
			{
				...
				$tmp = preg_replace('#[^0-9a-zA-Zа-яА-ЯёЁ\-_*|]#', ' ', $tmp);    // допустимые символы (кроме " которые отдельно)
				...
				$tmp = str_compact($tmp);
				// Sphinx escape End.
				$title_match_q = preg_replace('/[ёЁ]/', 'е', $tmp);
			} else {
...
if ($allowed_forums)
{
	...
	if (!$tr_error && !$tor_list_sql)
	{
	  ...
	  if (SPHINX_ENABLE)
	  {
		...
		if (!SPHINX_PECL)
		{
			include ($phpbb_root_path . 'includes/sphinxapi.php');
		}
        $sphinx = new SphinxClient ();
		$sphinx->SetConnectTimeout( (int) SPHINX_TIMEOUT );
        $sphinx->SetServer( (string) SPHINX_HOST, (int) SPHINX_PORT);
        $sphinx->SetMatchMode(SPH_MATCH_EXTENDED);
        $shpinx_query = '';

		if ($poster_id)
            $sphinx->SetFilter('topic_poster', Array ((int) $poster_id_sql));

        if ($only_new && !$req_poster_id)
            $sphinx->SetFilterRange('reg_time', 0, (int) $lastvisit, true);

		if ($prev_days && !$req_poster_id)
            $sphinx->SetFilterRange('reg_time', 0, (int) $reg_time, true);

        if ($s_not_seen && !$req_poster_id)
        {
            if ($s_not_seen_val == $never)
            {
                $sphinx->SetFilter('seeder_last_seen', Array((int) $s_seen_time));
            }
            else
            {
                $sphinx->SetFilter('seeder_last_seen', Array((int) 0), true);
                $sphinx->SetFilterRange('reg_time', 0, (int) $s_seen_time);
            }
        }

		if ($only_my)
            $sphinx->SetFilter('topic_poster', Array ((int) $user_id));

        if ($only_active && !$req_poster_id)
        {
            $sphinx->SetFilter('seeders', Array(0), true);
            $sphinx->SetFilter('leechers', Array(0), true);
        }
		
		if ($seed_exist && !$req_poster_id)
            $sphinx->SetFilter('seeders', Array(0), true);

        if (!empty($valid_forums) && !$req_poster_id)
            $sphinx->SetFilter('forum_id', $valid_forums);
        else
            $sphinx->SetFilter('forum_id', $allowed_forums);

		// IS GOLD || SILVER
		if ($is_gold == 1 && $is_silver == 1)
		{
			$sphinx->SetFilter('tor_type', array(1,2));
		} elseif ($is_gold == 1)
		{
			 $sphinx->SetFilter('tor_type', Array(1));
		} elseif ($is_silver == 1)
		{
			 $sphinx->SetFilter('tor_type', Array(2));
		}

        $sphinx->SetSortMode($sort_opt[$sort_val]['sql'] == 'ASC' ? SPH_SORT_ATTR_ASC : SPH_SORT_ATTR_DESC, $order_opt[$order_val]['field']);
		$sphinx->SetLimits(0,$tor_search_limit, $tor_search_limit);

        if (strpos ($title_match_q, '"') === FALSE) $title_match_q = preg_replace ('#[^\s]+-[^\s]+#', '"$0"', $title_match_q);

        if ( !$sphinx_result = $sphinx->Query($title_match_q, 'topics') )
        {
			//if (!$def)
			//{
			//	redirect(append_sid("$tracker_url?def=1", TRUE));
			//}
			//else
			//{
				message_die(GENERAL_ERROR, 'Could not query torrents information', '', __LINE__, __FILE__, $sphinx->GetLastError());
			//}
		}
        
        $tor_count = $sphinx_result['total'];
        
        if ($tor_count)
        {
            foreach ($sphinx_result['matches'] as $id => $topic)
            {
                $tor_list_ary[] = $id;
            }
            unset($sphinx_result);
	...
	else
	{
		...
		$passkey = $GLOBALS['db']->fetch_row("SELECT auth_key FROM ". BT_USERS_TABLE ." WHERE user_id = ". (int) $user_id ." LIMIT 1");
