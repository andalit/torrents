...
require_once VAR_DUMP_DIR .'Var_Dump/Renderer/Common.php';
...
class Var_Dump_Renderer
{
    ...
    {
        require_once VAR_DUMP_DIR .'Var_Dump/Renderer/' . $mode . '.php';
        $className = 'Var_Dump_Renderer_' . $mode;
        if (class_exists($className)) {
            $obj = new $className($options);
        } else {
            trigger_error('Var_Dump: renderer "' . $mode . '" not found');
            return NULL;
        }
        return $obj;
    }
